...
void InitUnigramTable() {
	//Table consists of unigram probability of words. Unigram is where the context depends on only one previous word.
  int a, i;
...
void ReadVocab() {
	// reads words from vocabulary file and sorts them.
  long long a, i = 0;
  char c;
  char word[MAX_STRING];
...
void InitNet() {
	// This function creates a neural network and calls createbinarytree() to create a huffman tree
  long long a, b;
...
void TrainModel() {
  ...
  // Initializes threads equal to the number of threads specified
  pthread_t *pt = (pthread_t *)malloc(num_threads * sizeof(pthread_t));
  printf("Starting training using file %s\n", train_file);
  starting_alpha = alpha;
  
  // If -read-vocab is enabled ReadVocab() to read the vocabulary or else learnt from training file
  if (read_vocab_file[0] != 0) ReadVocab(); else LearnVocabFromTrainFile();
  if (save_vocab_file[0] != 0) SaveVocab();
  if (output_file[0] == 0) return;
  
  // If negative sampling is enabled, Unigram table is created
  // threads are created and called TrainModelThread method for each thread
  InitNet();
  ...
  // If classes are not enabled simply vectors are written in output file or else, words are aggregated as classes.
  if (classes == 0) {
	...
	// K-means clustering is finding the k closest data points and clustering them into a class
...
int ArgPos(char *str, int argc, char **argv) {
	// The ArgPos is a helper function to iterate over the parameters and corresponding values
  int a;
...
int main(int argc, char **argv) {
  ...
  // If no argument is specified then this menu is presented
  if (argc == 1) {
  ...
  // Initializing the input vocab files array and output vectors file. As mentioned above, the vocab can be read and saved instead of obtaining from training data
  output_file[0] = 0;
  save_vocab_file[0] = 0;
  read_vocab_file[0] = 0;
  
  // Assigning all the values of parameters to corresponding variables. Explanation for each parameters are already present in menu
  // The ArgPos is a helper function to iterate over the parameters and corresponding values
  ...
  //Initializing the hash table and exp table by allocating memory using calloc()
  vocab = (struct vocab_word *)calloc(vocab_max_size, sizeof(struct vocab_word));
  vocab_hash = (int *)calloc(vocab_hash_size, sizeof(int));
  expTable = (real *)malloc((EXP_TABLE_SIZE + 1) * sizeof(real));
  
  // This creates a exptable with values e^x values
  for (i = 0; i < EXP_TABLE_SIZE; i++) {
    expTable[i] = exp((i / (real)EXP_TABLE_SIZE * 2 - 1) * MAX_EXP); // Precompute the exp() table
    expTable[i] = expTable[i] / (expTable[i] + 1);                   // Precompute f(x) = x / (x + 1)
  }
  
  // Training method is called.
  TrainModel();
  return 0;
}
